...
class CadastrarFilme:
    ...
    def verifica(self):
        ...
        if  Filmes.objects.filter(nome=self.__nome).exists():
                return mensagem[0]
        else:
...
class Editar_filme_criado:
    def __init__(self, id,nome,genero,link):
        ...
        self.__magnetc = get_object_or_404(MagnetcLinks, id=self.__id) 
...
class Excluir_filme_db:
    ...
    def confirm(self):
        ...
        get_object_or_404(MagnetcLinks, id=self.__id).delete()
